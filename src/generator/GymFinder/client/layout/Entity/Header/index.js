...
export default function EntityHeader (props) {
  ...
  const staticAttributes = ['id', 'title', 'profile_photo', 'cover_photo', 'rating', 'description', 'location', 'phone', 'email', 'updatedAt', 'createdAt'];
  const dynamicAttributes = Object.keys(props).filter(key => staticAttributes.indexOf(key) < 0);

  return (
    <div className={`EntityHeader`} style={styles.wrapper({ cover_photo })} >
      <Grid fluid>
